...
"""Extended unit test canadafire C module.  It requires data files
that are only on UAF Chinook system.
     pytest xtest_canadafire.py
"""
...
def test_canadafire():
    ...
    ROOT = '/import/FIREICE/fiboreal/Borries/files_to_Elizabeth'
    idir = os.path.join(ROOT, 'step2_ecmwfdata')
    odir = os.path.join(ROOT, 'step3_ecmwfdata')
    for year in range(1994,2020):
        print('YEAR: {}'.format(year))

        # Sample Inputs
        tin = read_data(os.path.join(idir, f'{year}_tmp2m_anom_wgt.txt'))
        hin = read_data(os.path.join(idir, f'{year}_rh_anom_wgt.txt'))
        win = read_data(os.path.join(idir, f'{year}_uv_wgt.txt'))
        rin = read_data(os.path.join(idir, f'{year}_precip_anom_wgt.txt'))

        # Previously computed outputs for sample inputs
        bui0 = read_data(os.path.join(odir, f'{year}_bui_anom_wgt.txt'))
        ffmc0 = read_data(os.path.join(odir, f'{year}_fm_anom_wgt.txt'))
        isi0 = read_data(os.path.join(odir, f'{year}_isi_anom_wgt.txt'))
        fwi0 = read_data(os.path.join(odir, f'{year}_fwi_anom_wgt.txt'))
        dmc0 = read_data(os.path.join(odir, f'{year}_dmc_anom_wgt.txt'))
        dc0 = read_data(os.path.join(odir, f'{year}_dc_anom_wgt.txt'))
